/*!
 *  @file Adafruit_TCS34725_SWwire.cpp
 *
 ...
 *  @param  value
 */
void Adafruit_TCS34725_SWwire::write8(uint8_t reg, uint8_t value) {
 ...
 *  @return value
 */
uint8_t Adafruit_TCS34725_SWwire::read8(uint8_t reg) {
 ...
 *  @return value
 */
uint16_t Adafruit_TCS34725_SWwire::read16(uint8_t reg) {
 ...
 */
void Adafruit_TCS34725_SWwire::enable() {
 ...
 */
void Adafruit_TCS34725_SWwire::disable() {
 ...
 *          Gain
 */
Adafruit_TCS34725_SWwire::Adafruit_TCS34725_SWwire(uint8_t it, tcs34725Gain_t gain) {
...
*/
boolean Adafruit_TCS34725_SWwire::begin(TwoWire* theWire, uint8_t addr = TCS34725_ADDRESS) {
  if (i2c_dev)
    delete i2c_dev;
  i2c_dev = new Adafruit_I2CDevice_Soft(theWire, addr);
  return init();
}

boolean Adafruit_TCS34725_SWwire::begin(SoftwareWire* theWire, uint8_t addr = TCS34725_ADDRESS) {
  if (i2c_dev)
    delete i2c_dev;
  i2c_dev = new Adafruit_I2CDevice_Soft(theWire, addr);
  return init();
 ...
 *  @return True if initialization was successful, otherwise false.
 */
boolean Adafruit_TCS34725_SWwire::init() {
  if (!i2c_dev->begin())
    return false;
 ...
 *          Integration Time
 */
void Adafruit_TCS34725_SWwire::setIntegrationTime(uint8_t it) {
  if (!_tcs34725Initialised) {
    Serial.println("ERRO\tTCS nao inicializado");
    return;
    //begin();
  }
 ...
 */
void Adafruit_TCS34725_SWwire::setGain(tcs34725Gain_t gain) {
  if (!_tcs34725Initialised) {
    Serial.println("ERRO\tTCS nao inicializado");
    return;
    //begin();
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_SWwire::getRawData(uint16_t *r, uint16_t *g, uint16_t *b, uint16_t *c) {
  if (!_tcs34725Initialised) {
    Serial.println("ERRO\tTCS nao inicializado");
    return;
    //begin();
 ...
 *          Clear channel value
 */
void Adafruit_TCS34725_SWwire::getRawDataOneShot(uint16_t *r, uint16_t *g, uint16_t *b,
                                          uint16_t *c) {
  if (!_tcs34725Initialised) {
    Serial.println("ERRO\tTCS nao inicializado");
    return;
 ...
 */
void Adafruit_TCS34725_SWwire::getRGB(float *r, float *g, float *b) {
 ...
 */
uint16_t Adafruit_TCS34725_SWwire::calculateColorTemperature(uint16_t r, uint16_t g, uint16_t b) {
 ...
 *  @return Color temperature in degrees Kelvin
 */
uint16_t Adafruit_TCS34725_SWwire::calculateColorTemperature_dn40(uint16_t r, uint16_t g, uint16_t b, uint16_t c) {
 ...
 *  @return Lux value
 */
uint16_t Adafruit_TCS34725_SWwire::calculateLux(uint16_t r, uint16_t g, uint16_t b) {
  float illuminance;
 ...
 */
void Adafruit_TCS34725_SWwire::setInterrupt(boolean i) {
 ...
 */
void Adafruit_TCS34725_SWwire::clearInterrupt() {
 ...
 *          High limit
 */
void Adafruit_TCS34725_SWwire::setIntLimits(uint16_t low, uint16_t high) {
